//
//  EditVehicleRequestVC.swift
//  IOCL
//
//  Created by neelam  on 23/03/23.
//
...
class EditVehicleRequestVC: UIViewController , WWCalendarTimeSelectorProtocol ,UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        CallForGetCarRequestByIdAPI(CM_REQUESTID : self.CM_REQUESTID)
    }
    ...
    func CallForvAddCarRequestAPI() {
        ...
        let url = "\(ApiLink.HOST_URL)\(UpdateCarRequest)"
        ...
        print(params)
        ServiceRequest.shared.UpdateCarRequestApi(params, completionHandler: { isSucess, response, errorMsg in
            KRProgressHUD.dismiss()
            let dict = response
            print("JSON: \(dict)")
    ...
    //MARK: - ADD CarRequest API CALLING FUNCTIONS ------------------------------------------
    func CallForGetCarRequestByIdAPI(CM_REQUESTID : Int) {
        KRProgressHUD.show()
        if reachability.isConnectedToNetwork() == false{
            KRProgressHUD.dismiss()
            self.createAlert(title: kAppName, message: ApiLink.INTERNET_ERROR_MESSAGE)
        ...
        ServiceRequest.shared.GetCarRequestByIdApi(params, completionHandler: { isSucess, response, errorMsg in
            ...
            if (statuscode == 200){
                ...
                self.selectTripTypeText.text =  data["CM_TRIPTYPE"] as? String
                let startDate =  data["CM_TRIP_STARTDT"] as? String
                let endDate  =  data["CM_TRIP_ENDDT"] as? String
                self.startDateTypeText.text = startDate?.convertToDateFormate(current: "yyyy-MM-dd'T'HH:mm:ss", convertTo: "dd/MM/YYYY") ?? ""
                self.endDateTypeText.text = endDate?.convertToDateFormate(current: "yyyy-MM-dd'T'HH:mm:ss", convertTo: "dd/MM/YYYY") ?? ""
                self.desText.text = data["CM_INTAPPROVER_REMARKS"] as? String
                self.vehicleModelAry.removeAll()
                for (i, item) in tripRequestList.enumerated() {
                    let obj = VehicleModelData()
                    obj.CT_DATE = item["CT_DATE"] as? String  ?? ""
                    obj.CT_TIME = item["CT_TIME"] as? String  ?? ""
                    obj.CT_PLACETO_REPORT = item["CT_PLACETO_REPORT"] as? String  ?? ""
                    obj.CT_PLACETO_VISIT = item["CT_PLACETO_VISIT"] as? String  ?? ""
                    self.vehicleModelAry.append(obj)
                }
                print(self.vehicleModelAry)
                self.tableView.reloadData()
            }else if (statuscode == 401){
...
extension EditVehicleRequestVC: UITableViewDataSource , UITableViewDelegate {
    //MARK: - TABLEV VIEW DATASOURCE -
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.Datetext.text = vehicleModelAry[indexPath.row].CT_DATE.convertToDateFormate(current: "yyyy-MM-dd'T'HH:mm:ss", convertTo: "dd/MM/YYYY")
...
extension EditVehicleRequestVC : UITextFieldDelegate {
